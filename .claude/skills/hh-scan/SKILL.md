...
## step-4: Отчёт юзеру — ТАБЛИЦА В ЧАТ (ОБЯЗАТЕЛЬНО)

**Таблицу печатать в чат всегда и до любого следующего шага** — не пересказом, не ссылкой на файл, не «топ-3 из них». Юзер решает по таблице, а не по твоему резюме (введено@2026-07-20: скоринг ушёл сразу в письма, юзер прервал — увидеть улов было негде).

Строки: все `score ≥ 5`, сортировка по score desc. Колонки строго:

| Score | Вакансия (ссылка на hh) | Компания | Вилка | Формат | Откликов | Тест | Почему (reason) |

Ниже таблицы — явным списком: кандидаты на отклик = `score ≥ 7` И `testPresent: false` И не `closed`; отдельной строкой — что отсеяно и по какой причине (`testPresent` → в `hh-test-apply`, стоп-лист, штрафы).

Сохранить `data/report-<дата>.md` (та же таблица + hook'и).
